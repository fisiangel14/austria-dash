  ...
  var getColors = function (item) {
    ...
    for (var i = 0; i < item.length; i++) {
      ...
      if ((item[i].VALUE !== null && item[i].VALUE <= item[i].OBJECTIVE) || (item[i].VALUE > item[i].OBJECTIVE && item[i].VALUE <= item[i].OBJECTIVE + item[i].TOLERANCE && item[i].TREND == 'Y')) {
        ...
        colors.ORANGE += 1;
      } else if ((item[i].VALUE === null || item[i].ERROR_CODE !== null) && item[i].DAYS_LATE > 1) {
        colors.NO_RESULT += 1;
      }
    }
    return colors;
  ...
  return function (items, inFilter) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (typeof inFilter.text === 'undefined' || item[0].METRIC_ID.toLowerCase().indexOf(inFilter.text.toLowerCase()) !== -1 || item[0].BILL_CYCLE === inFilter.text) {
        ...
        if (itemColors.GREEN && inFilter.GREEN) {
          filtered.push(item);
        } else if (itemColors.YELLOW && inFilter.YELLOW) {
          filtered.push(item);
        } else if (itemColors.ORANGE && inFilter.ORANGE) {
          filtered.push(item);
        } else if (itemColors.RED && inFilter.RED) {
          filtered.push(item);
        } else if (itemColors.NO_RESULT && inFilter.NO_RESULT) {
          filtered.push(item);
        }
      }
    }
...
angular.module('amxApp').filter('alarmFilter', function () {
  return function (items, inFilter) {
    ...
    for (var i = 0; i < (typeof items !== 'undefined' ? items.length : 0); i++) {
      ...
      if (inFilter.alarmLevel !== '%' && inFilter.alarmLevel.toUpperCase() !== item.SEVERITY.toUpperCase()) {
        continue;
      }

      if (inFilter.alarmSource !== '%' && inFilter.alarmSource !== 'JOB' && inFilter.alarmSource !== item.SOURCE) {
        continue;
      }

      if (inFilter.alarmSource === 'JOB' && (item.SOURCE === 'METRIC' || item.SOURCE === 'CONTROL' || item.SOURCE === 'DATO')) {
        continue;
      }

      if (inFilter.alarmStatus !== '%' && inFilter.alarmStatus !== 'Not closed' && inFilter.alarmStatus !== item.STATUS) {
        continue;
      }
      else if (inFilter.alarmStatus !== '%' && inFilter.alarmStatus === 'Not closed' && item.STATUS === 'Closed') {
        continue;
      }

      if (inFilter.isIncident && !item.INCIDENT_ID) {
        continue;
      }
      
      if (inFilter.searchText && inFilter.searchText.indexOf(':') > -1) {

      }
      else if (typeof inFilter.searchText === 'undefined' || item.ALARM_ID.toString().toLowerCase().indexOf(inFilter.searchText.toLowerCase()) !== -1 || item.OBJECT_ID.toLowerCase().indexOf(inFilter.searchText.toLowerCase()) !== -1 || item.ASSIGNED_TO.toLowerCase().indexOf(inFilter.searchText.toLowerCase()) !== -1) {
        filtered.push(item);
      }
    }
    return filtered;
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if ((inFilter.IMPLEMENTED && item.IMPLEMENTED === 'Y') || !inFilter.IMPLEMENTED) {
        ...
        if (typeof inFilter.text === 'undefined' || item.METRIC_ID.toLowerCase().indexOf(inFilter.text.toLowerCase()) !== -1 || item.AREA_ID.toLowerCase().indexOf(inFilter.text.toLowerCase()) !== -1) {
          if (item.GREEN && inFilter.GREEN && inFilter[item.FREQUENCY]) {
            filtered.push(item);
          } else if (item.YELLOW && inFilter.YELLOW && inFilter[item.FREQUENCY]) {
            filtered.push(item);
          } else if (item.ORANGE && inFilter.ORANGE && inFilter[item.FREQUENCY]) {
            filtered.push(item);
          } else if (item.RED && inFilter.RED && inFilter[item.FREQUENCY]) {
            filtered.push(item);
          } else if (item.NO_RESULT && inFilter.NO_RESULT && inFilter[item.FREQUENCY]) {
            filtered.push(item);
          }
        }
      }
...
angular.module('amxApp').filter('opcoFilter', function () {
  return function (items, opcoId) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      if (item.OPCO_ID == opcoId || opcoId === 0) {
        filtered.push(item);
      }
    }
    return filtered;
...
angular.module('amxApp').filter('notOpcoFilter', function () {
  return function (items, opcoId) {
    var filtered = [];
    if (typeof items === 'undefined') {
      return items;
...
angular.module('amxApp').filter('unsafe', function ($sce) {
