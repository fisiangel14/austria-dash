          ...
          sumsPS = _.map(_.groupBy($scope.data, function(e) { return e.PRODUCT_SEGMENT + e.RISK; }), 
            function(g) {
              return { 
                  ...
                  MONETARY_VALUE: _.reduce(g, function(m,x) { return m + Math.round(x.EUR_VALUE); }, 0),
                  MONETARY_RESIDUAL_VALUE: _.reduce(g, function(m,x) { return m + Math.round(x.EUR_VALUE)*(1-x.COVERAGE/100); }, 0),
                  MONETARY_COVERED_VALUE: _.reduce(g, function(m,x) { return m + Math.round(x.EUR_VALUE)*(x.COVERAGE/100); }, 0),
