...
export function getApiEndpoint(req, res, next) {
  ...
  if (req.params.apiEndpoint === "getInterfaceDatasources") {
    ...
    db.query(`
							...
							left join amx_system s on s.system_id = i.ifc_system_id
							left join amx_contact c on c.CONTACT_ID = i.ifc_contact_id
							where ds.OPCO_ID like case when ? = '0' then '%' else ifnull(?, '%') end and i.INTERFACE_ID = ?
							order by ds.NAME
    	`, 
    	[req.query.opcoId, req.query.opcoId, req.query.interfaceId],
    	function (err, row) {
	      if(err !== null) {
	      	console.log(err);
	        next(err);
