...
export function getApiEndpoint(req, res, next) {
  if (req.params.apiEndpoint === "getProcesses") {
  ...
  else if (req.params.apiEndpoint === "getProductSegmentsHistory") {
    db.query(	`SELECT h.* from cvg_history h
    				where 1=1
    					and h.OPCO_ID = ? 
    					and h.SNAPSHOT_DATE = (select max(SNAPSHOT_DATE) from cvg_history where SNAPSHOT_DATE <= ifnull(?, curdate()) )
				`,
    	[req.query.opcoId, req.query.forDate],
    	function (err, row) {
	      if(err !== null) {
	      	console.log(err);
	        next(err);
  ...
  else if (req.params.apiEndpoint === "getProductSegmentsForControlClone") {
    db.query(`SELECT 
								pg.PRODUCT_GROUP_ID, 
								ps.OPCO_ID, 
								pg.LOB, 
								...
								-- cvgGetProductSegmentMeasureCoverage(ps.PRODUCT_SEGMENT_ID) MEASURE_COVERAGE,
                'N' SELECTED,
                (select count(*) from cvg_risk_node where RISK_ID = ? and PRODUCT_SEGMENT_ID = ps.PRODUCT_SEGMENT_ID) PS_HAS_RISK_FLAG,
                (select count(*) from cvg_risk_node where RISK_ID = ? and SYSTEM_ID = ? and PRODUCT_SEGMENT_ID = ps.PRODUCT_SEGMENT_ID) PS_HAS_RISK_AND_SYSTEM_FLAG
							from cvg_product_group pg 
							left join cvg_product_segment ps on ps.PRODUCT_GROUP_ID = pg.PRODUCT_GROUP_ID and ps.OPCO_ID = ?
							order by pg.LOB, pg.PRODUCT_GROUP, ps.PRODUCT_SEGMENT 
								`,
    	[req.query.riskId, req.query.riskId, req.query.systemId, req.query.opcoId],
    	function (err, row) {
	      if(err !== null) {
	      	console.log(err);
	        next(err);
    ...
    db.query(`
							...
							left join v_cvg_control_system_mix s on s.CVG_CONTROL_ID = c.CONTROL_ID and s.SYSTEM_ID = ?
							where c.opco_id = ?
							order by 							
							case 
								when s.SYSTEM_ID is not null then 1 
  ...
  else if (req.params.apiEndpoint === "getRiskNodeSubRiskCoverage") {
    db.query(`
						select cvgGetRiskNodeSubRiskCoverage(?) COVERAGE
						`,
    ...
    async.parallel(
      {
        ...
        'CoverageStats': function(cb) {
          db.query(`
                select * 
                from v_cvg_control 
                where CONTROL_ID = ifnull(?, 0)
                `,
            [req.query.controlId],
            function (err, row) {
              if(err !== null) {
						...
						function(subRisk, forEachcallback) {
							...
							if (subRisk.RN_SUB_RISK_ID) {
								db.query(`update cvg_risk_node_sub_risk 
													set LIKELIHOOD=?, IMPACT=?, COVERAGE=?, FIXED=?, FIXED_COVERAGE=?
													where RN_SUB_RISK_ID=?`,
													[(subRisk.LIKELIHOOD?subRisk.LIKELIHOOD:0), (subRisk.IMPACT?subRisk.IMPACT:0), (subRisk.COVERAGE?subRisk.COVERAGE:0), (subRisk.FIXED?subRisk.FIXED:'N'), (subRisk.FIXED_COVERAGE?subRisk.FIXED_COVERAGE:0), subRisk.RN_SUB_RISK_ID],
													function(err, row) {
														if(err) {
														  console.log(err);
														}														
								...
								db.query(`insert into cvg_risk_node_sub_risk (RISK_NODE_ID, SUB_RISK_ID, LIKELIHOOD, IMPACT, COVERAGE, FIXED, FIXED_COVERAGE)
													values(?,?,?,?,?,?,?)`,
													[riskNode.RISK_NODE_ID, subRisk.SUB_RISK_ID, (subRisk.LIKELIHOOD?subRisk.LIKELIHOOD:0), (subRisk.IMPACT?subRisk.IMPACT:0), (subRisk.COVERAGE?subRisk.COVERAGE:0), (subRisk.FIXED?subRisk.FIXED:'N'), (subRisk.FIXED_COVERAGE?subRisk.FIXED_COVERAGE:0)],
													function(err, row) {
														if(err) {
														  console.log(err);
														}														
