	...
	if (req.params.apiEndpoint === "getAllProcedures") {
		...
		db.query(`
							SELECT 
								...
								cc.CONTROL_TYPE, cc.CONTROL_ASSERTION, cc.START_DATE, cc.LAST_REVIEW_DATE, cc.END_DATE, cc.CVG_CONTROL_ID, cc.IMPLEMENTATION_NOTES, cc.ESCALATION_NOTES,
								sc.SOX_RELEVANT, sc.SOLUTION_CONTACT_ID, 
								...
								case 
									when p.TYPE = 'J' then jc.DOCU_LINK
									when p.TYPE = 'S' then sc.DOCU_LINK
									else ''
								end DOCU_LINK																				
	...
	if (req.params.apiEndpoint === "getProcedure") {
		db.query(`
								SELECT 
									p.PROCEDURE_ID, p.OPCO_ID, p.TYPE, p.SUB_TYPE, p.STATUS_CODE, p.NAME, p.DESCRIPTION, p.MODIFIED,
									jc.CODE_LOCATION, jc.JOB_PARAMETERS, 
									cc.CONTROL_TYPE, cc.CONTROL_ASSERTION, cc.START_DATE, cc.LAST_REVIEW_DATE, cc.END_DATE, cc.CVG_CONTROL_ID, cc.IMPLEMENTATION_NOTES, cc.ESCALATION_NOTES, cc.BUSINESS_ASSURANCE_DOMAIN,
									sc.SOX_RELEVANT, sc.SOLUTION_CONTACT_ID, 
									s.SCHEDULE_ID, s.NAME SCHEDULE_NAME, s.TYPE SCHEDULE_TYPE, s.FREQUENCY, s.COMMENT SCHEDULE_COMMENT,
									case 
										when p.TYPE = 'J' then 'Job' 
...
export function postApiEndpoint(req, res, next) {
								...
								function (err, row) {
									if (!err) {
										...
										switch (req.body.TYPE) {
											case 'J':
																db.query(`insert into DFL_JOB_CATALOGUE (PROCEDURE_ID, CODE_LOCATION, JOB_PARAMETERS, DOCU_LINK) 
																					values (?,?,?,?)`,
																					[procedureId, req.body.CODE_LOCATION, req.body.JOB_PARAMETERS, req.body.DOCU_LINK],
																					function (err, row) {
																						if (!err) {
																...
																break;
											case 'C':
																db.query(`insert into DFL_CONTROL_CATALOGUE (PROCEDURE_ID, CONTROL_TYPE, CONTROL_ASSERTION, START_DATE, LAST_REVIEW_DATE, END_DATE, IMPLEMENTATION_NOTES, ESCALATION_NOTES, BUSINESS_ASSURANCE_DOMAIN) 
																					values (?,?,?,?,?,?,?,?,?)`,
																					[procedureId, req.body.CONTROL_TYPE, JSON.stringify(req.body.CONTROL_ASSERTION), (req.body.START_DATE?moment(req.body.START_DATE).format("YYYY-MM-DD"):null), (req.body.LAST_REVIEW_DATE?moment(req.body.LAST_REVIEW_DATE).format("YYYY-MM-DD"):null), (req.body.END_DATE?moment(req.body.END_DATE).format("YYYY-MM-DD"):null), req.body.IMPLEMENTATION_NOTES, req.body.ESCALATION_NOTES, JSON.stringify(req.body.BUSINESS_ASSURANCE_DOMAIN)],
																					function (err, row) {
																						if (!err) {
																							res.json({success: true})
																						}
			...
			db.query(`update DFL_PROCEDURE set OPCO_ID=?, TYPE=?, SUB_TYPE=?, NAME=?, DESCRIPTION=?, STATUS_CODE=?, SCHEDULE_ID=? 
								...
								function (err, row) {
									if (!err) {
										...
										switch (req.body.TYPE) {
											case 'J':
																db.query(`update DFL_JOB_CATALOGUE set CODE_LOCATION=?, JOB_PARAMETERS=?, DOCU_LINK=?
																					where PROCEDURE_ID = ?`,
																					[req.body.CODE_LOCATION, req.body.JOB_PARAMETERS, req.body.DOCU_LINK, req.body.PROCEDURE_ID],
																					function (err, row) {
																						if (!err) {
																							res.json({success: true})
																						}
																...
																break;
											case 'C':
																db.query(`update DFL_CONTROL_CATALOGUE set CONTROL_TYPE=?, CONTROL_ASSERTION=?, START_DATE=?, LAST_REVIEW_DATE=?, END_DATE=?, IMPLEMENTATION_NOTES = ?, ESCALATION_NOTES = ?, BUSINESS_ASSURANCE_DOMAIN=?
																					where PROCEDURE_ID = ?`,
																					[req.body.CONTROL_TYPE, JSON.stringify(req.body.CONTROL_ASSERTION), (req.body.START_DATE?moment(req.body.START_DATE).format("YYYY-MM-DD"):null), (req.body.LAST_REVIEW_DATE?moment(req.body.LAST_REVIEW_DATE).format("YYYY-MM-DD"):null), (req.body.END_DATE?moment(req.body.END_DATE).format("YYYY-MM-DD"):null), req.body.IMPLEMENTATION_NOTES, req.body.ESCALATION_NOTES, JSON.stringify(req.body.BUSINESS_ASSURANCE_DOMAIN), req.body.PROCEDURE_ID],
																					function (err, row) {
																						if (!err) {
																							res.json({success: true})
																						}
	...
	} else if (req.params.apiEndpoint === "cvgMapControlFromReference") {
			db.query(`call cvgMapControlFromReference(?, ?)`,
								[req.body.toControlId, req.body.fromControlId],
								function (err, row) {
									if (!err) {
										res.json({success: true})
									}
	...
	} else if (req.params.apiEndpoint === "cvgCloneControlToProductSegment") {
		
		async.forEach(req.body.toProductSegment, 
			function(ps, callback) {
				// console.log(req.body.fromControlId, req.body.fromRiskNodeId, ps.PRODUCT_SEGMENT_ID);

				db.query(`call cvgCloneControlToProductSegment(?, ?, ?)`,
				[req.body.fromControlId, req.body.fromRiskNodeId, ps.PRODUCT_SEGMENT_ID],
				function(err, row) {
					if (err) {
						console.log(err);
						res.json({success: false, error: err});				
...
export function deleteApiEndpoint(req, res, next) {
	...
	else if (req.params.apiEndpoint === "riskNodeMappingDelete")  {
		db.query('delete from cvg_risk_node_control where RISK_NODE_ID = ? and CONTROL_ID = ?', 
			[req.query.riskNodeId, req.query.controlId], 
			function (err, row) {
				if(err !== null) {
					console.log(err);
					res.json({success: false, error: err});
